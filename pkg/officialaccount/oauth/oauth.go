...
// NewOauth 实例化授权信息
func NewOauth(wxContext *wx_context.WxContext) *Oauth {
	auth := new(Oauth)
	auth.WxContext = wxContext
	return auth
}
...
func (oauth *Oauth) GetUserAccessToken(ctx context.Context, appId, appSecret, code string) (result ResAccessToken, err error) {
	urlStr := fmt.Sprintf(accessTokenURL, appId, appSecret, code)
	var response []byte
	response, err = util.HTTPGet(ctx, urlStr)
	if err != nil {
		return
	}
...
func (oauth *Oauth) RefreshAccessToken(ctx context.Context, refreshToken string) (result ResAccessToken, err error) {
...
func (oauth *Oauth) CheckAccessToken(ctx context.Context, accessToken, openID string) (b bool, err error) {
...
func (oauth *Oauth) GetUserInfo(ctx context.Context, accessToken, openID, lang string) (result UserInfo, err error) {
	if lang == "" {
		lang = "zh_CN"
	}
